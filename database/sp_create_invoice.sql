...
BEGIN
    ...
    SET @retailer_id = (SELECT retailer_id 
						FROM order_m 
    ...
    IF @retailer_id IS NOT NULL THEN
		...
		SET @product_ids = (SELECT GROUP_CONCAT(p.product_id)
							...
							WHERE om.order_m_id = order_id AND od.supplier_id = supplier_id AND it.invoice_item_id IS NULL);
		...
		IF @product_ids IS NOT NULL THEN
			...
			INSERT INTO invoice(supplier_id, retailer_id, created_date) VALUES(supplier_id, @retailer_id, CURRENT_TIMESTAMP());
			...
			WHERE om.order_m_id = order_id AND od.supplier_id = supplier_id;
